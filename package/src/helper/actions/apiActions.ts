...
// Replace deprecated '@playq' alias import with direct global exports
import { vars, comm } from "../../global";
import * as allure from "allure-js-commons";
import path from "path";
// Inline runner helpers to remove external alias dependency
...
export async function callApi(action: string, config: string, baseUrl: string, options?: string | Record<string, any>) {
  ...
  async function doCallApi() {
    ...
    const projectActionPath = path.resolve(process.cwd(), 'resources', 'api', `${action}.api.ts`);
    let actionPath: string;
        if (fs.existsSync(projectActionPath)) {
            actionPath = projectActionPath;
        }
        else {
            actionPath = require.resolve(`../../../../../../resources/api/${action}.api.ts`);
        }
