...
export async function waitForPageToLoad(
  ...
  try {
    // Use requestIdleCallback if available, else fallback to a short timeout (avoids window type error in TS context)
    await page.evaluate(() => {
      return new Promise((resolve) => {
  const ric: (cb: () => void) => any = (globalThis as any).requestIdleCallback || ((cb: () => void) => setTimeout(cb, 50));
  ric(() => resolve(true as any));
      });
    });
...
export async function waitForEnabled(
  ...
  // Best-effort: wait for element to appear first
  try { await locator.waitFor({ state: 'visible', timeout: Math.min(actionTimeout, 2000) }); } catch {}
  ...
  throw new Error(`Locator did not become enabled within ${actionTimeout}ms`);
}
...
export async function attachLog(
  ...
  if (isCucumberRunner()) {
    ...
    try {
      // Prefer Allure attachment in PW runner to avoid importing '@playwright/test' here
      const anyAllure: any = allure as any;
      if (typeof anyAllure.attachment === 'function') {
        anyAllure.attachment("Log", message, mimeType);
      } else if (typeof anyAllure.attach === 'function') {
        anyAllure.attach("Log", message, mimeType);
      } else {
        // Fallback to console
  ...
  async function doWaitForInputState() {
    ...
    while (Date.now() - start < actionTimeout) {
      try {
        ...
        if ((state === 'enabled' && enabled) || (state === 'disabled' && !enabled)) {
          break;
        }
      } catch {}
      await new Promise(res => setTimeout(res, 100));
    ...
    // Final check
    const finalEnabled = await target.isEnabled();
    if (!((state === 'enabled' && finalEnabled) || (state === 'disabled' && !finalEnabled))) {
      throw new Error(`❌ Input "${field}" did not reach state "${state}" within ${actionTimeout}ms.`);
    }
    ...
    try {
      if (match === "exact") {
        ...
        while (Date.now() - start < actionTimeout) {
          ...
          if (regexUrl.test(current)) break;
          await new Promise(res => setTimeout(res, 100));
        }
        const current = page.url();
        if (!regexUrl.test(current)) {
          throw new Error(`URL did not match ${regexUrl} within ${actionTimeout}ms. Last: ${current}`);
        }
      }
    } catch (error) {
  ...
  try {
    if (isCucumberRunner()) {
      ...
      if (world && typeof world.attach === "function") {
        ...
        persistScreenshotToDisk(screenshotBuffer, text);
      }
    } else if (isPlaywrightRunner()) {
      // Prefer Playwright test.info() attachment so both HTML and Allure reporters pick it up
      let attached = false;
      try {
        const { test } = require('@playwright/test');
        if (test && typeof test.info === 'function') {
          const info = test.info();
          if (info && typeof info.attach === 'function') {
            await info.attach(text || 'screenshot', { body: screenshotBuffer, contentType: 'image/png' });
            console.log(`✅ Screenshot attached via Playwright testInfo: ${text}`);
            attached = true;
          }
        }
      } catch {}
      ...
      if (!attached) {
        // Next, attempt Allure attachment API if available
        const anyAllure: any = allure as any;
        if (typeof anyAllure.attachment === 'function') {
          anyAllure.attachment(text, screenshotBuffer, "image/png");
          console.log(`✅ Screenshot attached via Allure: ${text}`);
          attached = true;
        } else if (typeof anyAllure.attach === 'function') {
          anyAllure.attach(text, screenshotBuffer, "image/png");
          console.log(`✅ Screenshot attached via Allure: ${text}`);
          attached = true;
        }
      }
      ...
      if (!attached) {
        console.warn(`⚠️ Could not attach screenshot via Playwright/Allure. Saving to disk instead.`);
        persistScreenshotToDisk(screenshotBuffer, text);
      }
    } else {
    ...
    try { persistScreenshotToDisk(screenshotBuffer, text); } catch {}
  }
}
